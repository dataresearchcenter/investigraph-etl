...
This framework tries to automate as many functionality (scheduling and executing workflows, monitoring, extracting and storing to various local or remote sources, configuration, ...) as possible with the help of [prefect.io](../stack/prefect.md).
...
Sqlite is only suitable for developing or small local deployments, better use a proper sql database that allows concurrent writes:
